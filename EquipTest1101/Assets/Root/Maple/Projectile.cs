...
public class Projectile : MonoBehaviour
{
    ...
    void PlayDamage_enemy(Collider2D collision)
    {
        ...
        if (collision.CompareTag("EnemyHitBox"))
        {
            ...
            m_targets.Add(obj);
            // obj.GetComponent<Enemy>().TakeDamage(m_damage);
            Debug.Log("obj.GetComponent<Enemy>().TakeDamage(m_damage);");
        }
    }
    void PlayDamage_player(Collider2D collision)
    {
        if (m_targets.Count >= m_targetMax)
            return;
